...
public class BoltSchedulerShouldReportFailureWhenBusyIT
{
    ...
    public void shouldReportFailureWhenAllThreadsInThreadPoolAreBusy() throws Throwable
    {
        ...
        try
        {
            connection3 = connectAndPerformBoltHandshake( cf.newInstance() );

            connection3.send( TransportTestUtil.chunk( init( "TestClient/1.1", emptyMap() ) ) );
            assertThat( connection3, TransportTestUtil.eventuallyReceives(
    ...
    private void enterStreaming( TransportConnection connection, int sleepSeconds ) throws Exception
    {
        ...
        connection.send( TransportTestUtil.chunk( init( "TestClient/1.1", emptyMap() ) ) );
        ...
        connection.send( TransportTestUtil.chunk( run( "UNWIND RANGE (1, 100) AS x RETURN x" ) ) );
        assertThat( connection, TransportTestUtil.eventuallyReceives( msgSuccess() ) );
    }
    ...
    {
        connection.send( TransportTestUtil.chunk( discardAll() ) );
